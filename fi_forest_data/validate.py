...
def validate_pipeline_config(cfg: dict, base_dir: Path | None = None) -> list[str]:
    ...
    if not isinstance(e, dict):
        ...
        if not isinstance(rusle, dict):
            ...
            _num(problems, rusle, "c_default", 0, 1, "module_e_plus.rusle")
            for mapname in ("k_by_soiltype", "c_by_clc"):
                m = rusle.get(mapname)
                if not isinstance(m, dict) or not m or not all(
                        isinstance(v, (int, float)) and 0 <= v <= 1 for v in m.values()):
                    problems.append(f"module_e_plus.rusle.{mapname}: expected a non-empty {{code: value in 0..1}} map")
